...
public class AuthCallbackAppServiceTests : ThirdPartyPlatformsApplicationTestBase
{
    ...
    private const string OfficialAuthorizerAppId = "wxf8b4f85f3a794e77";
    private const string MiniProgramAuthorizerAppId = "wx326eecacf7370d4e";
    private const string OfficialAuthorizationCode = "my_official_authorization_code";
    private const string MiniProgramAuthorizationCode = "my_miniprogram_authorization_code";
    ...
    protected override void AfterAddApplication(IServiceCollection services)
    {
        ...
        var weChatOpenPlatformApiRequester = Substitute.For<IWeChatOpenPlatformApiRequester>();
        services.Replace(ServiceDescriptor.Transient(s => weChatOpenPlatformApiRequester));

        weChatOpenPlatformApiRequester.RequestAsync(Arg.Any<string>(), HttpMethod.Post,
                Arg.Is<GetAuthorizerInfoRequest>(x =>
                    x.ComponentAppId == ThirdPartyPlatformsTestConsts.AppId &&
                    x.AuthorizerAppId == OfficialAuthorizerAppId))
            .Returns(
        ...
        weChatOpenPlatformApiRequester.RequestAsync(Arg.Any<string>(), HttpMethod.Post,
                Arg.Is<GetAuthorizerInfoRequest>(x =>
                    x.ComponentAppId == ThirdPartyPlatformsTestConsts.AppId &&
                    x.AuthorizerAppId == MiniProgramAuthorizerAppId))
            .Returns(
        ...
        _thirdPartyPlatformApiService.QueryAuthAsync(OfficialAuthorizationCode)
            .Returns(GetQueryAuthResponseAsync(OfficialAuthorizerAppId));

        _thirdPartyPlatformApiService.QueryAuthAsync(MiniProgramAuthorizationCode)
            .Returns(GetQueryAuthResponseAsync(MiniProgramAuthorizerAppId));
    }
    ...
    {
        await HandleCallbackAsync(OfficialAuthorizerAppId, OfficialAuthorizationCode);

        var authorizerWeChatApp = await _weChatAppRepository.FindAsync(x =>
            x.Type == WeChatAppType.Official && x.AppId == OfficialAuthorizerAppId);
    ...
    {
        await HandleCallbackAsync(MiniProgramAuthorizerAppId, MiniProgramAuthorizationCode);

        var authorizerWeChatApp = await _weChatAppRepository.FindAsync(x =>
            x.Type == WeChatAppType.MiniProgram && x.AppId == MiniProgramAuthorizerAppId);
    ...
    protected async Task HandleCallbackAsync(string authorizerAppId, string authorizationCode)
    {
        ...
        (await _authorizerAccessTokenCache.GetAsync(ThirdPartyPlatformsTestConsts.AppId,
            authorizerAppId)).ShouldBe(NewAuthorizerAccessToken);
    }
}